...
def quantidade_documentos_arquivados_tipo(
    documentos_arquivados: List[FileUpload], id_pessoal: int
) -> Dict[str, int]:
    """
    Conta a quantidade de documentos arquivados por tipo (sem índice
    numérico).
    """
    inicio_nome_arquivo = "DOCUMENTO"
    id_quatro_caracter = str(id_pessoal).zfill(4)
    ...
    for file in documentos_arquivados:
        ...
        tipo_limpo = tipo.rsplit("_", 1)[0] if "_" in tipo else tipo
        tipo_limpo = tipo_limpo.replace("_", " ")

        contagem_tipos[tipo_limpo] += 1

    return dict(contagem_tipos)
