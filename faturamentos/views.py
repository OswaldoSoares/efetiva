...
from minutas.models import Minuta, MinutaItens, MinutaFatura


def index_faturamento(request):
    fatura = MinutaFatura.objects.values('idMinuta_id__idCliente_id', 'idMinuta_id__idCliente_id__Fantasia').filter(
...
def minutas_faturar_cliente(request, idcli):
    minutas_faturar = MinutaFatura.objects.values(vMinuta=F('idMinuta_id__Minuta'), vData=F('idMinuta_id__DataMinuta'),
                                                  vValor=F('Valor')).filter(idMinuta_id__idCliente=idcli,
                                                                            idFatura=None, Valor__gt='0.00')
...
def cria_div_selecionada(request):
    ...
    minutafatura = MinutaFatura.objects.filter(idMinuta_id=minuta.idMinuta)
    context = {'minuta': minuta, 'minutaitens': minutaitens, 'minutafatura': minutafatura}
