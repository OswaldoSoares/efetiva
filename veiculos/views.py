...
from pessoas.models import Pessoal, FonePessoal
...
def indexveiculo(request):
    ...
    return render(
        request, "veiculos/index.html", {"veiculo": veiculo, "categoria": categoria}
    )
...
def consultaveiculo(request, idvei):
    ...
    docproprietario = FonePessoal.objects.filter(idPessoal_id=idproprietario)
    motorista = Pessoal.objects.filter(idPessoal=idmotorista)

    return render(
        request,
        "veiculos/consultaveiculo.html",
        {
            "veiculo": veiculo,
...
def excluiveiculo(request, idvei):
    ...
    if request.method == "POST":
        ...
        data["html_form"] = render_to_string(
            "veiculos/excluiveiculo.html", context, request=request
        )
...
def atualiza_categoria_veiculos(request, id):
    ...
    if form.is_valid():
        ...
        return redirect("lista_categoria_veiculos")
    return render(
        request, "veiculos/criacategoria.html", {"form": form, "categoria": categoria}
    )
