...
def index_minuta(request):
    ...
    if meu_filtro_minuta:
        ...
        if meu_filtro_status == "CONCLUIDA":
            ...
            minuta = Minuta.objects.filter(StatusMinuta=meu_filtro_status).order_by(
                "-Minuta"
            )
    else:
...
def consultaminuta(request, idmin):
    ...
    motorista_da_minuta = MinutaColaboradores.objects.filter(
        idMinuta=idmin, Cargo="MOTORISTA"
    ).annotate(TipoPgto=F("idPessoal__TipoPgto"))
    ajudantes_da_minuta = MinutaColaboradores.objects.filter(
        idMinuta=idmin, Cargo="AJUDANTE"
    ).annotate(TipoPgto=F("idPessoal__TipoPgto"))
    total_de_ajudantes = MinutaColaboradores.objects.filter(
        idMinuta=idmin, Cargo="AJUDANTE"
    ).count()
    itens_paga_motorista = MinutaItens.objects.filter(
        idMinuta=idmin, TipoItens="PAGA"
    ).exclude(Descricao="AJUDANTE")
    soma_paga_motorista = str(
        sum([i["Valor"] for i in itens_paga_motorista.values("Valor")])
    )
    ...
    if tabelaveiculo:
        ...
        valorporcentagemrecebe = list(
            tabelaveiculo.values("PorcentagemCobra")[0].values()
        )[0]
        valorporcentagempaga = list(
            tabelaveiculo.values("PorcentagemPaga")[0].values()
        )[0]
        ...
        valorentregakg_recebe = list(
            tabelaveiculo.values("EntregaKGCobra")[0].values()
        )[0]
        valorentregavolume_recebe = list(
            tabelaveiculo.values("EntregaVolumeCobra")[0].values()
        )[0]
        ...
        valorentregavolume_paga = list(
            tabelaveiculo.values("EntregaVolumePaga")[0].values()
        )[0]
    ...
    formcomentarios = CadastraComentarioMinuta(
        initial={"idMinuta": "29", "Comentarios": comentarios}
    )
    ...
    if notas_minuta:
        ...
        formsaidaextraajudante = CadastraMinutaSaidaExraAjudante(
            instance=minutanotasform[0]
        )
    notas_minuta_guia = MinutaNotas.objects.filter(
        idMinuta=idmin, NotaGuia="0"
    ).order_by("Nota")
    notas_perimetro = (
        MinutaNotas.objects.values("Cidade")
        .filter(idMinuta=idmin)
    ...
    totalvalornotas = MinutaNotas.objects.filter(idMinuta=idmin).aggregate(
        totalvalor=Sum("ValorNota")
    )
    totalpesonotas = MinutaNotas.objects.filter(idMinuta=idmin).aggregate(
        totalpeso=Sum("Peso")
    )
    ...
    totalquantidadenotas = MinutaNotas.objects.filter(
        idMinuta=idmin, NotaGuia="0"
    ).count()
    ...
    itensminuta = MinutaItens.objects.filter(
        idMinuta=idmin, TipoItens="DESPESA"
    ).order_by("Descricao")
    veiculo = ""
    ...
    minuta_itens_fechada = MinutaItens.objects.filter(idMinuta=idmin).order_by(
        "TipoItens", "Descricao"
    )
    ...
    if totalhoras > dezhoras:
        ...
        calcula_ajudante_recebe_hora_extra += (
            valor_ajudante_recebe_hora_extra * Decimal(int(horas) - 10)
        )
    ...
    values_tabela_recebe = [
        ...
        valorkmrecebe,
        valorentrega_recebe,
        valorentregakg_recebe,
        valorentregavolume_recebe,
        valorsaidarecebe,
        valorcapacidaderecebe,
        porceperimetrorecebe,
        0,
        valorajudanterecebe,
        0,
    ]
    ...
    values_minuta_recebe = [
        None,
        totalvalornotas["totalvalor"],
        totalvalornotas["totalvalor"],
        minimohoras,
        excedehoras,
        totalkm,
    ...
    values_tabela_paga = [
        valorporcentagempaga,
        valorhorapaga,
        100,
        valorkmpaga,
        valorentrega_paga,
        valorentregakg_paga,
        valorentregavolume_paga,
        valorsaidapaga,
        valorcapacidadepaga,
        porceperimetropaga,
        0,
    ...
    for index, item in enumerate(keys_recebe):
        ...
        tabela_recebe_e_paga[str(item)]["id_swr"] = (
            "sw-%s-recebe" % keys_nome_recebe[index]
        )
        tabela_recebe_e_paga[str(item)]["name_swr"] = "switch"
        tabela_recebe_e_paga[str(item)]["checked_swr"] = switch_recebe.get(
            "%s" % keys_nome_recebe[index]
        )
        tabela_recebe_e_paga[str(item)]["value_swr"] = (
            "sw-%s-recebe" % keys_nome_recebe[index]
        )
        tabela_recebe_e_paga[str(item)]["class_swr"] = (
            "switch switch--shadow change-%s-recebe" % keys_nome_recebe[index]
        )
        tabela_recebe_e_paga[str(item)]["type_swr"] = "checkbox"
        # Coluna 3 Recebe - tipo moeda ou porcentagem
        tabela_recebe_e_paga[str(item)]["type_tabela_recebe"] = type_tabela_recebe[
            index
        ]
        # Columa 3 Recebe - valores das tabelas
        tabela_recebe_e_paga[str(item)]["id_tr"] = (
            "ta-%s-recebe" % keys_nome_recebe[index]
        )
        tabela_recebe_e_paga[str(item)]["name_tr"] = "tabela-recebe"
        tabela_recebe_e_paga[str(item)]["value_tr"] = values_tabela_recebe[index]
        tabela_recebe_e_paga[str(item)]["class_tr"] = (
            "demonstrativo-input change-%s-recebe" % keys_nome_recebe[index]
        )
        ...
        tabela_recebe_e_paga[str(item)]["type_minuta_recebe"] = type_minuta_recebe[
            index
        ]
        # Columa 4 Recebe - valores das tabelas
        tabela_recebe_e_paga[str(item)]["id_mr"] = (
            "mi-%s-recebe" % keys_nome_recebe[index]
        )
        tabela_recebe_e_paga[str(item)]["name_mr"] = "minuta-recebe"
        tabela_recebe_e_paga[str(item)]["value_mr"] = values_minuta_recebe[index]
        tabela_recebe_e_paga[str(item)]["class_mr"] = (
            "demonstrativo-input change-%s-recebe" % keys_nome_recebe[index]
        )
        ...
        tabela_recebe_e_paga[str(item)]["id_tor"] = (
            "to-%s-recebe" % keys_nome_recebe[index]
        )
        # Coluna 5 Recebe - totais input hidden
        tabela_recebe_e_paga[str(item)]["id_hir"] = (
            "hi-%s-recebe" % keys_nome_recebe[index]
        )
    ...
    for index, item in enumerate(keys_paga):
        ...
        tabela_recebe_e_paga[str(item)]["checked_swp"] = switch_paga.get(
            "%s" % keys_nome_paga[index]
        )
        tabela_recebe_e_paga[str(item)]["value_swp"] = (
            "sw-%s-paga" % keys_nome_paga[index]
        )
        tabela_recebe_e_paga[str(item)]["class_swp"] = (
            "switch switch--shadow change-%s-paga" % keys_nome_paga[index]
        )
        ...
        tabela_recebe_e_paga[str(item)]["class_tp"] = (
            "demonstrativo-input change-%s-paga" % keys_nome_paga[index]
        )
        ...
        tabela_recebe_e_paga[str(item)]["class_mp"] = (
            "demonstrativo-input change-%s-paga" % keys_nome_paga[index]
        )
...
def imprimeminuta(request, idmin):
    if request.user.is_authenticated:
        ...
        pdf.drawString(
            convertemp(56), convertemp(279), "TRANSEFETIVA TRANSPORTE - EIRELLI - ME"
        )
        ...
        pdf.setFont("Times-Roman", 12)
        pdf.drawString(
            convertemp(70), convertemp(268), "(11) 2305-0582 - WHATSAPP (11) 94167-0583"
        )
        pdf.drawString(
            convertemp(67),
            convertemp(263),
        ...
        pdf.drawString(
            convertemp(11), convertemp(246), "CLIENTE: " + str(minuta.idCliente.Nome)
        )
        endereco = minuta.idCliente.Endereco + " - " + minuta.idCliente.Bairro
        if len(endereco) > 45:
            pdf.drawString(
                convertemp(11), convertemp(242), "ENDEREÇO: " + endereco[0:45] + "..."
            )
        else:
            pdf.drawString(
                convertemp(11),
        ...
        pdf.drawString(
            convertemp(11), convertemp(234), "INSCRIÇÃO CNPJ:" + minuta.idCliente.CNPJ
        )
        pdf.drawString(
            convertemp(11),
            convertemp(230),
        ...
        if contato:
            pdf.drawString(
                convertemp(11), convertemp(226), "CONTATO: " + contato[0].Contato
            )
            pdf.drawString(
        ...
        pdf.setFillColor(HexColor("#c1c1c1"))
        pdf.rect(
            convertemp(105), convertemp(249), convertemp(95), convertemp(5), fill=1
        )
        pdf.setFillColor(HexColor("#000000"))
        ...
        if motorista:
            y -= 4
            pdf.drawString(
                convertemp(106), convertemp(y), "MOTORISTA: {}".format(motorista)
            )
        if ajudante:
            if ajudante.count() == 1:
                y -= 4
                ...
                for x in range(ajudante.count()):
                    ...
                    if x == 0:
                        ...
                        pdf.drawString(
                            convertemp(126), convertemp(y), str(ajudante[x].idPessoal)
                        )
        if minuta.KMInicial:
            y -= 4
            pdf.drawString(
                convertemp(106), convertemp(y), "KM Inicial: " + str(minuta.KMInicial)
            )
        y -= 4
        pdf.drawString(
            convertemp(106),
        ...
        pdf.setFont("Times-Roman", 10)
        pdf.setFillColor(HexColor("#c1c1c1"))
        pdf.rect(convertemp(10), convertemp(212), convertemp(95), convertemp(5), fill=1)
        pdf.setFillColor(HexColor("#000000"))
        pdf.drawCentredString(
            convertemp(57.5), convertemp(213.3), "DESCRIÇÃO DO SERVIÇO EXECUTADO"
        )
        # ----
        ...
        pdf.setFillColor(HexColor("#c1c1c1"))
        pdf.rect(
            convertemp(10), convertemp(167), convertemp(190), convertemp(5), fill=1
        )
        pdf.setFillColor(HexColor("#000000"))
        pdf.drawCentredString(
        ...
        pdf.drawString(
            convertemp(118), convertemp(12), "DATA, ASSINATURA E CARIMBO DO CLIENTE"
        )
...
def estorna_minuta(request, idmin):
    ...
    if minuta.StatusMinuta == "ABERTA":
        ...
        altera_status_minuta("ABERTA", idmin)
        itens_minuta_recebe_excluir = MinutaItens.objects.filter(idMinuta=idmin).filter(
            TipoItens="RECEBE"
        )
        for itens in itens_minuta_recebe_excluir:
            excluiminutaitens(itens.idMinutaItens)
        itens_minuta_paga_excluir = MinutaItens.objects.filter(idMinuta=idmin).filter(
            RecebePaga="P"
        )
...
def criaminutamotorista(request):
    ...
    return salva_form(request, form, "minutas/criaminutamotorista.html", idminuta)
...
def criaminutaajudante(request):
    ...
    return salva_form(request, form, "minutas/criaminutaajudante.html", idminuta)
...
def criaminutadespesa(request):
    if float(request.POST.get("Valor")) > 0.00:
        if request.method == "POST":
            ...
            if despesa:
                idminutaitem = list(despesa.values("idMinutaItens")[0].values())[0]
                minutaitens = get_object_or_404(MinutaItens, idMinutaItens=idminutaitem)
                form = CadastraMinutaDespesa(request.POST, instance=minutaitens)
            else:
                form = CadastraMinutaDespesa(request.POST)
            return salva_form(request, form, "minutas/colsultaminuta.html", idminuta)
    return redirect("consultaminuta", request.POST.get("idMinuta"))


def buscaminutadespesa(descricao, idminuta):
    buscadespesa = MinutaItens.objects.filter(Descricao=descricao, idMinuta=idminuta)
    return buscadespesa
...
def buscaminutaentrega(request):
    nota_guia = MinutaNotas.objects.filter(
        idMinuta_id=request.GET.get("id_minuta"), Nota=request.GET.get("nota_guia")
    )
...
def adiciona_romaneio_minuta(request):
    ...
    id_min = request.GET.get("idMinuta")
    facade.save_notas_romaneio_minuta(id_rom, id_min)
    data = facade.create_data_adiciona_romaneio_minuta(request, id_min)
    data = retorna_json(data)
    return data
